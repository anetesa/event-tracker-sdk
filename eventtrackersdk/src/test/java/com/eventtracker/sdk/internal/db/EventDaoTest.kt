...
/**
 * Подход: Robolectric + настоящая in-memory база Room (не фейк, не мок).
 *
 * Почему: здесь проверяется сам SQL — сортировка, `DELETE ... WHERE`, `GROUP BY`, поведение при
 * равенстве timestamp — то есть логика, которую выполняет SQLite, а не код приложения. Такое
 * нельзя достоверно проверить ни на [FakeEventDao] (он вообще не выполняет SQL, а эмулирует
 * поведение вручную на коллекциях), ни на моке (некого мокать — это тест самого запроса).
 * Robolectric даёт настоящий SQLite-движок на JVM без эмулятора/устройства, поэтому это
 * единственный тест в проекте, где расходы на его настройку оправданы.
 */
